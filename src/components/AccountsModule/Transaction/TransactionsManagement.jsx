  ...
  if (isLoading) {
    return (
      <div className="p-6 min-h-screen bg-gradient-to-br from-purple-50 via-blue-50 to-indigo-100">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 mb-6">{Array.from({length:3}).map((_,i)=>(<div key={i} className="bg-white rounded-xl shadow-sm border p-5"><div className="flex items-center justify-between mb-3"><div className="animate-pulse bg-gray-200 rounded w-24 h-3" /><div className="animate-pulse bg-gray-200 rounded-full w-8 h-8" /></div><div className="animate-pulse bg-gray-200 rounded w-16 h-7 mb-2" /><div className="animate-pulse bg-gray-200 rounded w-32 h-3" /></div>))}</div>
        <div className="bg-white rounded-xl shadow-sm border overflow-hidden"><div className="bg-gray-50 border-b px-6 py-4"><div className="flex gap-4">{Array.from({length:6}).map((_,i)=>(<div key={i} className="animate-pulse bg-gray-200 rounded w-24 h-3" />))}</div></div>{Array.from({length:8}).map((_,i)=>(<div key={i} className={`px-6 py-4 flex gap-4 ${i%2===1?'bg-gray-50/50':''} border-b border-gray-100`}>{Array.from({length:6}).map((_,j)=>(<div key={j} className="animate-pulse bg-gray-200 rounded w-24 h-4" />))}</div>))}</div>
      </div>
    );
  }
      ...
      <div className="bg-white rounded-2xl shadow-xl border border-gray-100">
        <div className="p-6 border-b border-gray-100">
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
            ...
            <button
              onClick={() => exportTransactionsExcel(filteredTransactions, { transactorName: searchTerm })}
